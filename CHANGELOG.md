...
- **`/openapi.json`: the API as a machine-readable document.** `/api.md` has
  ...
  `/openapi.json` — OpenAPI 3.1, 80 operations, generated by `npm run openapi`
  from the code that already defines the API rather than written beside it.
  Request bodies come from the same zod schemas that validate the requests, so
  a body in the document cannot disagree with what the server accepts; paths
  and methods are read out of the routers, which is the only place they exist;
  and `tests/openapi.test.ts` regenerates the document and fails when the
  committed copy is behind, the way `docs/schema.md` already works. A new route
  fails the suite until someone writes it a summary.

  Two things it deliberately does not carry. Response bodies are typed as
  objects, because the DTOs are TypeScript interfaces rather than schemas and a
  second hand-maintained source of truth for them would be worse than a pointer
  to `/api.md`. What a role may do is not in it either: capabilities are
  middleware an organiser can change while the event is running, so the honest
  answer is the bundle's `permissions` map at runtime, and the document says so
  rather than freezing a copy. `/api.md`, `/agents.md`, `/llms.txt` and
  `/robots.txt` all point at it, and `/agents.md` no longer lists "OpenAPI
  document: none yet" among this instance's capabilities.
