...
| An organiser pointing this server at a relay of their choosing | The relay list is the organiser's, up to ten `wss://` URLs, and the server opens an outbound WebSocket to each: an organiser can make the instance connect to any host it can reach, including one on its own network, and *Send a test* echoes each connection's error text back to them. Accepted: the organiser already holds the event, `ws://` is refused in production, nothing is ever read back from a relay, and what goes out is the programme they chose to publish. An instance on a private network that must not be probed from inside puts egress rules in front of it, as it would for any outbound hook |
...
| Nostr signing key (`nsec`) | The event's identity on Nostr: publishing and retracting in its name, for good | Generated by **Enable**, or pasted in with **Import key**; shown by **Export key** (organiser, `auth` rate limit, one audit row each) | Forever; encrypted at rest, decrypted only to sign and to export | Nothing. **Import key** gives the event a *new* identity; the old key stays valid in every hand that holds it | Un-publish anything, or stop a holder of the old key posting as the old identity |
...
## What a Nostr relay sees

Enabling Nostr is the second **publication decision** an organiser can make,
and the wider one: a Telegram group has members, a relay has readers, and
relays copy each other. The switch on Manage → Publish lists what follows
before it can be ticked.

What leaves, as calendar entries under the event's own key and kept current
with the database: for every session that is not a draft, not deleted and
not opted out — title, description as written, start and end, room, event
name, format, tags, speaker display names, livestream links and the link to
the session page; for the event — its name, dates and link, as the calendar
and the profile. As notes, when the trigger is ticked: what the Telegram
section lists, plus a pitch's title, its pitcher's display name and one line
of its description under `pitched`, which is off by default. What does not
leave: contributions, stars, interest, drafts, breaks, anyone's UID,
password or role, the audit log, the people list, and anything whose
*Publish to Nostr* box its author or an organiser unticked — that one is
named in no note either.

Two things differ from Telegram:

- **Livestream links go with the calendar entry whenever a session has
  them.** There is no separate switch as there is for Telegram (migration
  024); a stream address a session carries is on the relays for as long as
  the session is. An organiser who treats the stream address as the key to
  the room keeps it out of the session or keeps the session off Nostr.
- **Nothing is recallable, and the app does not pretend otherwise.** A
  calendar entry is replaced by its next version and retracted by a deletion
  request (NIP-09) that a relay may honour; a note is never edited and never
  deleted. Unticking a session's box, deleting it, **Retract everything** and
  switching off all send deletion requests or stop the next post; none of
  them reaches a copy a relay or a reader already made.

The signing key is the identity; its handling is in the threat model above
