...
describe('the gate has a door marked speaker', () => {
  ...
  it('opens on that form, with the reason, when a speaker link failed', () => {
    expect(gate).toContain('speakerLinkFailed?: boolean');
    expect(gate).toContain("useState<LinkMode>(speakerLinkFailed ? 'speaker' : 'none')");
    expect(gate).toContain('speakerLinkFailed ? SPEAKER_CODE_FAILED : null');
  });
...
describe('a speaker link signs the device in on arrival', () => {
  ...
  it('settles before the gate or the schedule is drawn', () => {
    const settle = schedule.indexOf("speakerLink.status === 'waiting'");
    const gateDraw = schedule.indexOf("data.status === 'gate'");
    expect(settle).toBeGreaterThan(-1);
    expect(settle).toBeLessThan(gateDraw);
