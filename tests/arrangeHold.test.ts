...
describe('a dropped block waits for the server', () => {
  it('demands a promise from onMove, so a void wrapper cannot compile', () => {
    const prop = calendar.slice(
      calendar.indexOf('onMove: ('),
      calendar.indexOf('export function Calendar'),
    );
  ...
  it('releases the hold only when that promise settles', () => {
    const settle = calendar.slice(
      calendar.indexOf('const settle = '),
      calendar.indexOf('const onUp = '),
    );
    expect(settle).toContain('result: Promise<void>');
    expect(settle).toMatch(
      /result\.finally\(\(\) => \{[\s\S]*setDrag\(\(d\) => \(d\?\.id === session\.id \? null : d\)\);/,
    );
  });
