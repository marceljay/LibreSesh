...
/**
 * The demo login page hands out roles on a click, so it has no password box — but it
 ...
 * so the login page opened on three dead controls with nothing saying why, and an
...
const source = readFileSync(
  join(import.meta.dirname, '..', 'web', 'src', 'components', 'Login.tsx'),
  'utf8',
);

/** The demo branch only: everything between `{demo ? (` and its `) : (`. */
const demoBranch = (() => {
  const start = source.indexOf('{demo ? (');
...
describe('the demo login page asks for a name before it offers a role', () => {
  ...
  it('still refuses to enter without one', () => {
    ...
    expect(source).toContain("const nameMissing = name.trim() === '';");
  });
