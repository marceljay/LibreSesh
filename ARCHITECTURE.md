...
through the container's Chromium, no network). `web/public/openapi.json` is
kept the same way by `npm run openapi` — see §The files the instance serves for
programs.
...
### The files the instance serves for programs
...
catch-all that hands `index.html` to every other path. Four prose documents
live there on purpose, and they are four rather than one because the
conventions that grew up around agent access settled into layers, each
...
Two more files sit beside them, machine-readable rather than prose, and
outside the layering:

| Served at | |
| --- | --- |
| [`/openapi.json`](web/public/openapi.json) | The same endpoints as an OpenAPI 3.1 document, for a client generator rather than a reader. Generated — see below |
| [`/robots.txt`](web/public/robots.txt) | Crawler directives: `Disallow: /e/`, since every event is behind a password and only the landing page is written for a stranger. Its comments point at the documents above, because an agent fetching it reads the whole file |

`/openapi.json` is generated by `npm run openapi` from the routers and the zod
schemas, the same arrangement as `docs/schema.md`: a committed artefact, and a
test that regenerates it and fails when the copy is behind. It carries what the
code knows — paths, methods, request bodies, success statuses — and deliberately
not what it does not. Response bodies are typed as objects, because the DTOs are
TypeScript interfaces rather than schemas and a second source of truth for them
would be worse than a pointer to `api.md`. Capabilities are absent entirely: an
organiser can change the permission matrix while the event runs, so the document
tells the reader to read the bundle's `permissions` map instead of freezing a
copy of it.
...
content. `GET /api/events` already lists the events on a box; nothing served
from `web/public/` should add to what that gives away.
...
endpoint is added without a line in it. It also checks the cross-links,
`SKILL.md`'s frontmatter and `robots.txt`'s pointers — the things a careless
edit silently breaks. `tests/openapi.test.ts` does the same job for the
generated document, and rather more of it: it walks the routers a second time
by a different route and compares the two sets, so a new endpoint fails the
suite until it is both in the document and given a summary by hand.
