...
| `GET`/`PATCH`/`DELETE /telegram`, `POST /telegram/code`, `POST /telegram/test` | The event's Telegram group: status, mode and lead time, disconnecting, minting a bind code, and a test message. Organisers |
