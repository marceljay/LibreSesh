...
- **Permission integrity — prove the layer between predicate and pixel.**
  Plan: `_planning/plans/2026-09-08-permission-integrity.md`. Phases 1 and
  2, the admin-route sweep and the rendered-controls sweep, are in
  CHANGELOG. Left: (3) role changes pushed live to the affected page, as
  permission changes already are — today a demoted attendee keeps buttons
  the server refuses until reload. The "Finer permissions" item below is
  the plan's optional phase 4 and waits on it.
